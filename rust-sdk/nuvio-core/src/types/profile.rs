...
mod tests {
    ...
    fn test_profile_with_details() {
        ...
        assert_eq!(profile.avatar_url, Some("https://example.com/avatars/john.jpg".to_string()));
        assert_eq!(profile.is_kids, false);
    ...
    fn test_profile_serde_roundtrip() {
        ...
        // Deserialize back
        let deserialized: Profile = serde_json::from_str(&json).expect("Failed to deserialize Profile");
    ...
    fn test_profile_maturity_ratings() {
        ...
        let g_rated = Profile::with_details(
            "g".to_string(), "G User".to_string(), None, true, None, None, None, Some("G".to_string()),
        );
        assert_eq!(g_rated.maturity_rating, Some("G".to_string()));

        let pg_rated = Profile::with_details(
            "pg".to_string(), "PG User".to_string(), None, false, None, None, None, Some("PG".to_string()),
        );
        assert_eq!(pg_rated.maturity_rating, Some("PG".to_string()));

        let pg13_rated = Profile::with_details(
            "pg13".to_string(), "PG-13 User".to_string(), None, false, None, None, None, Some("PG-13".to_string()),
        );
        assert_eq!(pg13_rated.maturity_rating, Some("PG-13".to_string()));

        let r_rated = Profile::with_details(
            "r".to_string(), "R User".to_string(), None, false, None, None, None, Some("R".to_string()),
        );
