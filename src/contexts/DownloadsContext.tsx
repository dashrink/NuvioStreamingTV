...
function isDownloadableUrl(url: string): boolean {
  ...
  const isStreamingFormat = streamingFormats.some(
    format =>
...
export const DownloadsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const maybeNotifyProgress = useCallback(async (d: DownloadItem) => {
    try {
      ...
      await notificationService.notifyDownloadProgress(
        d.title,
        d.progress,
        d.downloadedBytes,
        d.totalBytes
    ...
    async (id: string) => {
        ...
        const progressCallback = (data: any) => {
          ...
          if (current) {
            maybeNotifyProgress({
              ...current,
              downloadedBytes: totalBytesWritten,
              totalBytes: totalBytesExpectedToWrite || current.totalBytes,
              progress: totalBytesExpectedToWrite
                ? Math.floor((totalBytesWritten / totalBytesExpectedToWrite) * 100)
                : current.progress,
            });
      ...
      try {
        ...
        if (currentItem && currentItem.status === 'paused') {
          console.log(
            `[DownloadsContext] Download was paused during resume, keeping paused state: ${id}`
          );
        ...
        try {
          ...
          console.log(
            `[DownloadsContext] File validation passed: ${result.uri} (${fileInfo.size} bytes)`
          );
        ...
        if (currentItem && currentItem.status === 'paused') {
          console.log(
            `[DownloadsContext] Error was due to pause, keeping paused state and resumable: ${id}`
          );
          // Keep resumable for next resume attempt - DO NOT DELETE
          return;
        }
        ...
        if (e.message && e.message.includes('validation failed')) {
          console.log(
            `[DownloadsContext] Validation error - clearing resume data for fresh start: ${id}`
          );
          updateDownload(id, d => ({
            ...d,
            status: 'error',
          ...
          e.message &&
          (e.message.includes('size mismatch') || e.message.includes('empty'))
        ) {
          // File corruption detected - clear everything for fresh start
          console.log(
            `[DownloadsContext] File corruption detected - clearing for fresh start: ${id}`
          );
          updateDownload(id, d => ({
            ...d,
            status: 'error',
          ...
          console.log(
            `[DownloadsContext] Network/other error - keeping resume data for retry: ${id}`
          );
          updateDownload(id, d => ({ ...d, status: 'error', updatedAt: Date.now() }));
  ...
  const startDownload = useCallback(
    async (input: StartDownloadInput) => {
      ...
      if (!isDownloadableUrl(input.url)) {
        throw new Error(
          'This stream format cannot be downloaded. M3U8 (HLS) and DASH streaming formats are not supported for download.'
        );
      }
      ...
      const baseId =
        input.type === 'series' && input.season && input.episode
      ...
      const baseDir =
        (FileSystem as any).documentDirectory || (FileSystem as any).cacheDirectory || '/tmp/';
      const uniqueId = `${Date.now()}_${Math.random().toString(36).substring(7)}`;
      const extension = await getExtensionFromHeaders(input.url, input.headers);
      const fileUri = extension
        ? `${baseDir}downloads/${uniqueId}.${extension}`
        : `${baseDir}downloads/${uniqueId}`;
      ...
      try {
        ...
        if (currentItem && currentItem.status === 'paused') {
          console.log(
            `[DownloadsContext] Download was paused during initial download, keeping paused state: ${compoundId}`
          );
          ...
          try {
            ...
            console.log(
              `[DownloadsContext] Could not get savable state after pause: ${compoundId}`,
              savableError
            );
          }
        ...
        if (current && current.status === 'paused') {
          console.log(
            `[DownloadsContext] Error was due to pause during initial download, keeping paused state and resumable: ${compoundId}`
          );
          // CRITICAL FIX: Save resumeData when paused
          try {
            const savableState = resumable.savable();
            ...
            console.log(
              `[DownloadsContext] Could not get savable state after pause error: ${compoundId}`,
              savableError
            );
          }
          // Don't delete resumable - keep it for resume
        ...
        if (e.message && e.message.includes('validation failed')) {
          console.log(
            `[DownloadsContext] Validation error - clearing resume data for fresh start: ${compoundId}`
          );
          updateDownload(compoundId, d => ({
            ...d,
            status: 'error',
          ...
          // File corruption detected - clear everything for fresh start
          console.log(
            `[DownloadsContext] File corruption detected - clearing for fresh start: ${compoundId}`
          );
          updateDownload(compoundId, d => ({
            ...d,
            status: 'error',
          ...
          // Network or other errors - keep resume data for retry
          console.log(
            `[DownloadsContext] Network/other error - keeping resume data for retry: ${compoundId}`
          );
    ...
    async (id: string) => {
      ...
      if (resumable) {
        try {
          ...
          console.log(
            `[DownloadsContext] Pause async failed (this is normal if already paused): ${id}`,
            error
          );
        ...
        console.log(
          `[DownloadsContext] No resumable found for download: ${id}, just marked as paused`
        );
      }
    },
    [updateDownload]
