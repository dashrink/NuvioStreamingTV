...
export class TrailerService {
  ...
  private static readonly ENV_XPRIME_URL =
    process.env.EXPO_PUBLIC_XPRIME_URL || 'https://db.xprime.tv/trailers';
  ...
  static async getTrailerUrl(
    ...
    logger.info(
      'TrailerService',
      `getTrailerUrl requested: title="${title}", year=${year}, tmdbId=${tmdbId || 'n/a'}, type=${type || 'n/a'}, useLocal=${this.USE_LOCAL_SERVER}`
    );
    if (this.USE_LOCAL_SERVER) {
      ...
      logger.info(
        'TrailerService',
        `Local server failed, falling back to XPrime for: ${title} (${year})`
      );
   ...
   * @returns Promise<string | null> - The trailer URL or null if not found
   */
  private static async getTrailerFromLocalServer(
    title: string,
    year: number,
    tmdbId?: string,
    type?: 'movie' | 'tv'
    ...
    try {
      ...
      logger.info(
        'TrailerService',
        `Local server response: status=${response.status} ok=${response.ok} content-type=${contentType} elapsedMs=${elapsed}`
      );
      ...
      if (!response.ok) {
        logger.warn(
          'TrailerService',
          `Auto-search failed: ${response.status} ${response.statusText}`
        );
        return null;
      }
      ...
      logger.info(
        'TrailerService',
        `Successfully found trailer: ${String(data.url).substring(0, 80)}...`
      );
      return data.url;
    } catch (error) {
  ...
  private static isValidTrailerUrl(url: string): boolean {
    try {
      ...
      const hasVideoFormat =
        /\.(mp4|m3u8|mpd|webm|mov|avi|mkv)$/i.test(urlObj.pathname) ||
  ...
  static getBestFormatUrl(url: string): string {
    ...
    if (url.includes('formats=')) {
      ...
      if (url.includes('MPEG4')) {
        ...
        logger.info(
          'TrailerService',
          `Optimized format URL from MPEG4: ${best.substring(0, 80)}...`
        );
        return best;
      }
    }
  ...
  static async isTrailerAvailable(title: string, year: number): Promise<boolean> {
    ...
    logger.info(
      'TrailerService',
      `Trailer availability for ${title} (${year}): ${trailerUrl ? 'available' : 'not available'}`
    );
  ...
  static async getTrailerFromYouTubeUrl(
    ...
    try {
      ...
      logger.info(
        'TrailerService',
        `Direct trailer response: status=${response.status} ok=${response.ok}`
      );

      if (!response.ok) {
        logger.warn(
          'TrailerService',
          `Direct trailer failed: ${response.status} ${response.statusText}`
        );
        return null;
      }
      ...
      logger.info(
        'TrailerService',
        `Successfully got direct trailer: ${String(data.url).substring(0, 80)}...`
      );
      return data.url;
    } catch (error) {
      if (error instanceof Error && error.name === 'AbortError') {
  ...
  static async testServers(): Promise<{
    ...
    try {
      ...
      if (response.ok || response.status === 404) {
        ...
        logger.info(
          'TrailerService',
          `Local server online. Response time: ${results.localServer.responseTime}ms`
        );
      }
    } catch (error) {
      const msg = error instanceof Error ? `${error.name}: ${error.message}` : String(error);
    ...
    try {
      ...
      if (response.ok || response.status === 404) {
        ...
        logger.info(
          'TrailerService',
          `XPrime server online. Response time: ${results.xprimeServer.responseTime}ms`
        );
      }
    } catch (error) {
      const msg = error instanceof Error ? `${error.name}: ${error.message}` : String(error);
    ...
    logger.info(
      'TrailerService',
      `Server test results -> local: ${results.localServer.status}, xprime: ${results.xprimeServer.status}`
    );
    return results;
  }
}
