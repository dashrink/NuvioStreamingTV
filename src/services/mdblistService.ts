...
export class MDBListService {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      logger.log(
        '[MDBListService] Initialized with API key:',
  ...
  async getRatings(imdbId: string, mediaType: 'movie' | 'show'): Promise<MDBListRatings | null> {
    ...
    if (this.ratingsCache.has(cacheKey)) {
      ...
      logger.log(
        `[MDBListService] Retrieved ${cachedRatings ? 'ratings' : 'negative result'} from cache for ${mediaType}:`,
        imdbId
      );
    ...
    try {
      ...
      logger.log(
        `[MDBListService] Starting to fetch ${ratingTypes.length} different rating types in parallel`
      );
            ...
            if (response.status === 403) {
              ...
              try {
                ...
                if (errorJson.error === 'Invalid API key') {
                  ...
                  if (
                    this.apiKeyErrorCount === 1 ||
                    this.apiKeyErrorCount % 5 === 0 ||
                  ...
                  logger.warn(
                    `[MDBListService] 403 Forbidden, but not invalid key error:`,
                    errorJson
                  );
                }
              } catch (parseError) {
              ...
              logger.warn(
                `[MDBListService] Failed to fetch ${ratingType} rating. Status:`,
                response.status
              );
              const errorText = await response.text();
      ...
      logger.log(
        `[MDBListService] Stored ${result ? 'ratings' : 'negative result'} in cache for ${mediaType}:`,
        imdbId
      );
  ...
  invalidateCache(imdbId: string, mediaType: 'movie' | 'show'): void {
    ...
    logger.log(
      `[MDBListService] Cache entry ${hadEntry ? 'invalidated' : 'not found'} for ${mediaType}:`,
      imdbId
    );
  }
