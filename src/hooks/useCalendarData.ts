  ...
  const fetchCalendarData = useCallback(
    async (forceRefresh = false) => {
      ...
      try {
        ...
        if (!forceRefresh) {
          const cachedData = await robustCalendarCache.getCachedCalendarData(libraryItems, {
        ...
        const addSeries = (
          ...
          year: number,
          poster: string,
          source: 'watchlist' | 'continue-watching' | 'watched' | 'library'
        ) => {
          if (!addedIds.has(id)) {
            addedIds.add(id);
            allSeries.push({
          ...
          if (watchlistShows) {
            for (const item of watchlistShows) {
              if (item.show && item.show.ids.imdb) {
                addSeries(item.show.ids.imdb, item.show.title, item.show.year, '', 'watchlist');
              }
            }
          }
        }
        ...
        for (const item of librarySeries) {
          addSeries(item.id, item.name, item.year || 0, item.poster, 'library');
        }
        ...
        if (traktAuthenticated && watchedShows) {
          ...
          for (const item of recentWatched) {
            if (item.show && item.show.ids.imdb) {
              addSeries(item.show.ids.imdb, item.show.title, item.show.year, '', 'watched');
            }
          }
        }
        ...
        if (allSeries.length > maxSeries) {
          logger.warn(
            `[CalendarData] Too many series (${allSeries.length}), limiting to ${maxSeries} to prevent memory issues`
          );
          ...
          async (series: StreamingContent, index: number) => {
            try {
              ...
              if (episodeData && episodeData.episodes.length > 0) {
                ...
                const tmdbEpisodes: { [key: string]: any } = {};
                ...
                if (tmdbId && episodeData.episodes.length > 0) {
                  try {
                    ...
                    logger.warn(
                      `[CalendarData] TMDB fetch failed for ${series.name}, continuing without additional metadata`
                    );
                  }
                }
        ...
        allEpisodes.sort((a, b) => {
          try {
            ...
            logger.warn(
              `[CalendarData] Error sorting episodes: ${a.releaseDate} vs ${b.releaseDate}`,
              error
            );
        ...
        const thisWeekEpisodes = await memoryManager.filterLargeArray(allEpisodes, ep => {
          try {
            ...
            logger.warn(
              `[CalendarData] Error parsing date for this week filtering: ${ep.releaseDate}`,
              error
            );
            return false;
          }
        ...
        const upcomingEpisodes = await memoryManager.filterLargeArray(allEpisodes, ep => {
          try {
            if (!ep.releaseDate) return false;
            const parsed = parseISO(ep.releaseDate);
            ...
            logger.warn(
              `[CalendarData] Error parsing date for upcoming filtering: ${ep.releaseDate}`,
              error
            );
            return false;
          }
        ...
        const recentEpisodes = await memoryManager.filterLargeArray(allEpisodes, ep => {
          try {
            if (!ep.releaseDate) return false;
            const parsed = parseISO(ep.releaseDate);
            ...
            logger.warn(
              `[CalendarData] Error parsing date for recent filtering: ${ep.releaseDate}`,
              error
            );
            return false;
          }
        ...
        logger.log(
          `[CalendarData] Episode categorization: This Week: ${thisWeekEpisodes.length}, Upcoming: ${upcomingEpisodes.length}, Recently Released: ${recentEpisodes.length}, Series without episodes: ${seriesWithoutEpisodes.length}`
        );

        // Debug: Show some example episodes from each category
        if (thisWeekEpisodes && thisWeekEpisodes.length > 0) {
          logger.log(
            `[CalendarData] This Week examples:`,
            thisWeekEpisodes.slice(0, 3).map(ep => ({
              title: ep.title,
              date: ep.releaseDate,
        ...
        if (recentEpisodes && recentEpisodes.length > 0) {
          logger.log(
            `[CalendarData] Recently Released examples:`,
        ...
        if (thisWeekEpisodes.length > 0) {
          ...
          logger.log(
            `[CalendarData] Added 'This Week' section with ${thisWeekEpisodes.length} episodes`
          );
        }
        if (upcomingEpisodes.length > 0) {
          sections.push({ title: 'Upcoming', data: upcomingEpisodes });
          logger.log(
            `[CalendarData] Added 'Upcoming' section with ${upcomingEpisodes.length} episodes`
          );
        }
        if (recentEpisodes.length > 0) {
          sections.push({ title: 'Recently Released', data: recentEpisodes });
          logger.log(
            `[CalendarData] Added 'Recently Released' section with ${recentEpisodes.length} episodes`
          );
        }
        if (seriesWithoutEpisodes.length > 0) {
          sections.push({
            title: 'Series with No Scheduled Episodes',
            data: seriesWithoutEpisodes,
          });
          logger.log(
            `[CalendarData] Added 'Series with No Scheduled Episodes' section with ${seriesWithoutEpisodes.length} episodes`
          );
        }
        ...
        sections.forEach((section, index) => {
          logger.log(
            `  Section ${index}: "${section.title}" with ${section.data?.length || 0} episodes`
          );
        });
        ...
        await robustCalendarCache.setCachedCalendarData(sections, libraryItems, {
          watchlist: watchlistShows,
          continueWatching,
          watched: watchedShows,
        });
        ...
        await robustCalendarCache.setCachedCalendarData(
          [],
          libraryItems,
          { watchlist: watchlistShows, continueWatching, watched: watchedShows },
          true
        );
      } finally {
