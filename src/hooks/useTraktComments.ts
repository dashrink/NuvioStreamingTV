  ...
  const loadComments = useCallback(
    async (pageNum: number = 1, append: boolean = false) => {
      ...
      try {
        ...
        console.log(
          `[useTraktComments] Loading comments for ${type} - IMDb: ${imdbId}, TMDB: ${tmdbId}, page: ${pageNum}`
        );

        switch (type) {
          case 'movie':
            fetchedComments = await traktService.getMovieComments(
              imdbId,
              tmdbId,
              pageNum,
              COMMENTS_PER_PAGE
            ...
            break;
          case 'show':
            fetchedComments = await traktService.getShowComments(
              imdbId,
              tmdbId,
              pageNum,
              COMMENTS_PER_PAGE
          ...
          case 'season':
            if (season !== undefined) {
              fetchedComments = await traktService.getSeasonComments(
                imdbId,
                season,
                pageNum,
                COMMENTS_PER_PAGE
          ...
          case 'episode':
            if (season !== undefined && episode !== undefined) {
              fetchedComments = await traktService.getEpisodeComments(
                imdbId,
                season,
                episode,
                pageNum,
        ...
        setComments(prevComments => {
          if (append) {
            ...
            console.log(
              `[useTraktComments] Appended ${fetchedComments.length} comments, total: ${newComments.length}`
            );
            return newComments;
          } else {
