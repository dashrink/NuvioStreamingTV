    ...
    [{ nativeEvent: { translationY: volumeGestureTranslateY } }],
    {
      ...
      listener: (event: PanGestureHandlerGestureEvent) => {
        ...
        if (state === State.ACTIVE) {
          ...
          if (
            Math.abs(translationY) < 5 &&
            Math.abs(lastVolumeGestureY.current - translationY) > 20
          ) {
          ...
          const newVolume = Math.max(
            volumeRange.min,
            Math.min(volumeRange.max, config.volume + volumeChange)
          );
          ...
          if (config.debugMode) {
            console.log(
              `[GestureControls] Volume set to: ${newVolume} (Platform: ${Platform.OS}, Sensitivity: ${volumeSensitivity})`
            );
          }
    ...
    [{ nativeEvent: { translationY: brightnessGestureTranslateY } }],
    {
      ...
      listener: (event: PanGestureHandlerGestureEvent) => {
        ...
        if (state === State.ACTIVE) {
          ...
          if (
            Math.abs(translationY) < 5 &&
            Math.abs(lastBrightnessGestureY.current - translationY) > 20
          ) {
          ...
          if (config.debugMode) {
            console.log(
              `[GestureControls] Device brightness set to: ${newBrightness} (Platform: ${Platform.OS}, Sensitivity: ${brightnessSensitivity})`
            );
          }
