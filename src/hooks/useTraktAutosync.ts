...
export function useTraktAutosync(options: TraktAutosyncOptions) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      logger.log(
        `[TraktAutosync] Component unmount #${unmountCount.current} for: ${sessionKey.current}`
      );
    };
  ...
  const handlePlaybackStart = useCallback(
    async (currentTime: number, duration: number) => {
      ...
      logger.log(
        `[TraktAutosync] handlePlaybackStart called: time=${currentTime}, duration=${duration}, authenticated=${isAuthenticated}, enabled=${autosyncSettings.enabled}, alreadyStarted=${hasStartedWatching.current}, alreadyStopped=${hasStopped.current}, sessionComplete=${isSessionComplete.current}, session=${sessionKey.current}`
      );

      if (!isAuthenticated || !autosyncSettings.enabled) {
        logger.log(
          `[TraktAutosync] Skipping handlePlaybackStart: authenticated=${isAuthenticated}, enabled=${autosyncSettings.enabled}`
        );
        return;
      }
      ...
      if (isSessionComplete.current) {
        logger.log(
          `[TraktAutosync] Skipping handlePlaybackStart: session is complete, preventing any restart`
        );
        return;
      }
      ...
      if (hasStopped.current) {
        logger.log(
          `[TraktAutosync] Skipping handlePlaybackStart: session already stopped, preventing restart`
        );
        return;
      }
      ...
      if (hasStartedWatching.current) {
        logger.log(
          `[TraktAutosync] Skipping handlePlaybackStart: already started=${hasStartedWatching.current}`
        );
        return;
      }
      ...
      try {
        ...
        if (success) {
          ...
          logger.log(
            `[TraktAutosync] Started watching: ${contentData.title} (session: ${sessionKey.current})`
          );
        }
      } catch (error) {
  ...
  const handleProgressUpdate = useCallback(
    async (currentTime: number, duration: number, force: boolean = false) => {
      if (isUnmounted.current) return; // Prevent execution after component unmount
      ...
      try {
        ...
        if (force) {
          ...
          if (success) {
            ...
            logger.log(
              `[TraktAutosync] IMMEDIATE: Progress updated to ${progressPercent.toFixed(1)}%`
            );
          }
        } else {
  ...
  const handlePlaybackEnd = useCallback(
    async (
      currentTime: number,
      duration: number,
      reason: 'ended' | 'unmount' | 'user_close' = 'ended'
    ) => {
      if (isUnmounted.current) return; // Prevent execution after component unmount
      ...
      if (isSessionComplete.current) {
        logger.log(
          `[TraktAutosync] Session already complete, skipping end call (reason: ${reason})`
        );
        return;
      }
      ...
      if (hasStopped.current) {
        ...
        if (progressImprovement > 5) {
          logger.log(
            `[TraktAutosync] Session already stopped, but progress improved significantly by ${progressImprovement.toFixed(1)}% (${lastSyncProgress.current.toFixed(1)}% → ${currentProgressPercent.toFixed(1)}%), allowing update`
          );
      ...
      if (!isSignificantUpdate && now - lastStopCall.current < debounceThreshold) {
        logger.log(
          `[TraktAutosync] Ignoring duplicate stop call within ${debounceThreshold}ms (reason: ${reason})`
        );
        return;
      }
      ...
      try {
        ...
        if (reason === 'unmount') {
          ...
          try {
            ...
            if (savedProgress && savedProgress.duration > 0) {
              const savedProgressPercent = Math.min(
                100,
                Math.max(0, (savedProgress.currentTime / savedProgress.duration) * 100)
              );
        ...
        if (success) {
          ...
          if (progressPercent >= autosyncSettings.completionThreshold) {
            ...
            logger.log(
              `[TraktAutosync] Session marked as complete (scrobbled) at ${progressPercent.toFixed(1)}%`
            );
          ...
          logger.log(
            `[TraktAutosync] ${useImmediate ? 'IMMEDIATE: ' : ''}Successfully stopped watching: ${contentData.title} (${progressPercent.toFixed(1)}% - ${reason})`
          );
        } else {
        ...
        if (reason === 'ended' || progressPercent >= 80) {
          ...
          logger.log(
            `[TraktAutosync] Reset session state for ${reason} at ${progressPercent.toFixed(1)}%`
          );
        }
      } catch (error) {
