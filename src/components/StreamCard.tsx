    ...
    const handleDownload = useCallback(async () => {
      ...
      try {
        ...
        try {
          ...
          if (
            downloadsModule &&
            downloadsModule.isDownloadingUrl &&
            downloadsModule.isDownloadingUrl(url)
          ) {
            showAlert(
              'Already Downloading',
              'This download has already started for this exact link.'
            );
            return;
          }
        } catch {}
        ...
        const inferredTitle =
          parentTitle || stream.name || stream.title || parent.metaName || 'Content';
        const inferredType: 'movie' | 'series' =
          parentType || (parent.kind === 'series' || parent.type === 'series' ? 'series' : 'movie');
        const season =
          typeof parentSeason === 'number' ? parentSeason : parent.season || parent.season_number;
        const episode =
          typeof parentEpisode === 'number'
            ? parentEpisode
            : parent.episode || parent.episode_number;
        ...
        const provider =
          ...
          parent.addonName ||
          parent.addonId ||
          (stream.addonName as any) ||
          (stream.addonId as any) ||
          'Provider';

        // Use parentId first (from route params), fallback to stream metadata
        const idForContent =
          parentId || parent.imdbId || parent.tmdbId || parent.addonId || inferredTitle;
      ...
      >
        ...
        {showLogos && scraperLogo && (
          <View style={styles.scraperLogoContainer}>
            {scraperLogo.toLowerCase().endsWith('.svg') ||
            scraperLogo.toLowerCase().includes('.svg?') ? (
              <Image
        ...
        <View style={styles.streamDetails}>
          ...
          <View style={styles.streamMetaRow}>
            {streamInfo.isDolby && <QualityBadge type="VISION" />}
        ...
        {settings?.enableDownloads !== false && (
          <TouchableOpacity
            style={[
              styles.streamAction,
              { marginLeft: 8, backgroundColor: theme.colors.elevation2 },
            ]}
