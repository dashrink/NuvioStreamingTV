  ...
  (
    ...
    useEffect(() => {
      if (paused !== undefined) {
        ...
        logger.info(
          'TrailerPlayer',
          `External paused prop changed: ${paused}, setting isPlaying to ${!paused}`
        );
      }
    }, [paused]);
        ...
        <Video
          ...
          source={(() => {
            const androidHeaders =
              Platform.OS === 'android' ? { 'User-Agent': 'Nuvio/1.0 (Android)' } : ({} as any);
            // Help ExoPlayer select proper MediaSource
            const lower = (trailerUrl || '').toLowerCase();
            const looksLikeHls =
              /\.m3u8(\b|$)/.test(lower) || /hls|applehlsencryption|playlist|m3u/.test(lower);
          ...
          style={[styles.video, contentType === 'movie' && styles.movieVideoScale]}
          resizeMode="cover"
          paused={!isPlaying}
          repeat={false}
          muted={isMuted}
                  ...
                  <View style={styles.controlButtons}>
                    ...
                    {onFullscreenToggle && (
                      <TouchableOpacity style={styles.controlButton} onPress={onFullscreenToggle}>
                        <MaterialIcons name="fullscreen" size={isTablet ? 32 : 24} color="white" />
                      </TouchableOpacity>
                    )}
                  </View>
                </View>
...
const styles = StyleSheet.create({
  ...
  movieVideoScale: {
    transform: [{ scale: 1.3 }], // Custom scale for movies to crop black bars
  },
  videoOverlay: {
    position: 'absolute',
    top: 0,
