...
const HeroCarousel: React.FC<HeroCarouselProps> = ({ items, loading = false }) => {
  ...
  const baseCardWidthForHeight = useMemo(() => Math.min(windowWidth * 0.8, 480), [windowWidth]);
  ...
  if (loading) {
    return (
      <View
        style={[styles.container, { paddingTop: 12 + effectiveTopOffset }] as StyleProp<ViewStyle>}
      >
  ...
  const BackgroundImage = React.memo(
    ({ item, insets }: { item: StreamingContent; insets: any }) => {
      return (
        <View
          style={[styles.backgroundContainer, { top: -insets.top }] as StyleProp<ViewStyle>}
          pointerEvents="none"
        >
            ...
            {Platform.OS === 'android' ? (
              ...
              <>
                ...
                {Platform.OS === 'ios' && GlassViewComp && liquidGlassAvailable ? (
                  <GlassViewComp style={styles.backgroundImage as any} glassEffectStyle="regular" />
                ) : (
                  <BlurView style={styles.backgroundImage as any} intensity={30} tint="dark" />
                )}
              </>
            )}
            <LinearGradient
  ...
  return (
    <View>
      <Animated.View
        style={[styles.container as ViewStyle, { paddingTop: 12 + effectiveTopOffset }]}
      >
        ...
        {settings.enableHomeHeroBackground && data[activeIndex] && (
          <BackgroundImage item={data[activeIndex]} insets={insets} />
        )}
    ...
    return (
      <View style={{ width: cardWidth + 16 }}>
        ...
        >
          ...
          {item.logo && (
            <View
              style={{ position: 'absolute', left: 0, right: 0, bottom: 40, alignItems: 'center' }}
            >
...
const CarouselCard: React.FC<CarouselCardProps> = memo(
  ({
    ...
    useEffect(() => {
      isFlipped.value = withTiming(flipped ? 1 : 0, {
        duration: 300,
            ...
            {isTablet ? (
              <>
                <View style={styles.bannerContainer as ViewStyle}>
                  {!bannerLoaded && <View style={styles.skeletonBannerFull as ViewStyle} />}
                ...
                <View style={styles.backContent as ViewStyle}>
                  {item.logo && !logoFailed ? (
                    ...
                    <Text
                      style={[styles.backTitle as TextStyle, { color: colors.highEmphasis }]}
                      numberOfLines={1}
                    >
                      {item.name}
                    </Text>
                  ...
                  {item.year && (
                    <View style={styles.infoRow as ViewStyle}>
                      <View style={styles.infoItem as ViewStyle}>
                        ...
                        <Text
                          style={[styles.infoText as TextStyle, { color: colors.mediumEmphasis }]}
                        >
                          {item.year}
                        </Text>
                      </View>
                  ...
                  <ScrollView
                    style={{
                      maxHeight: 120,
                      width: Math.round(cardWidth * 0.85),
                      alignSelf: 'center',
                    }}
              ...
              <>
                {/* FRONT FACE */}
                <Animated.View
                  style={[styles.flipFace as any, styles.frontFace as any, frontFlipStyle]}
                        ...
                        <View>
                          <Text
                            style={[
                              ...
                              { color: colors.highEmphasis, textAlign: 'center' },
                            ]}
                            numberOfLines={1}
                          >
                            {item.name}
                    ...
                    {item.genres && (
                      <View style={styles.genresOverlay as ViewStyle} pointerEvents="none">
                        <View>
                          <Animated.Text
                            style={[
                              styles.genres as TextStyle,
                              { color: colors.mediumEmphasis, textAlign: 'center' },
                              overlayAnimatedStyle,
                            ]}
                            numberOfLines={1}
                          >
                ...
                {/* BACK FACE */}
                <Animated.View
                  style={[styles.flipFace as any, styles.backFace as any, backFlipStyle]}
                ...
                <View style={styles.flipButtonContainer as ViewStyle} pointerEvents="box-none">
                  ...
                  >
                    <Ionicons
                      name={flipped ? 'close' : 'information-outline'}
                      size={18}
                      color={colors.white}
                    />
                  </TouchableOpacity>
