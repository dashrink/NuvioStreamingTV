...
export const RecentSearches: React.FC<RecentSearchesProps> = ({
  ...
  return (
    <View style={styles.container}>
      <Text style={[styles.title, { color: currentTheme.colors.white }]}>Recent Searches</Text>
      {recentSearches.map((search, index) => (
        <TouchableOpacity key={index} style={styles.searchItem} onPress={() => handlePress(search)}>
          <MaterialIcons
            name="history"
            size={20}
            ...
            style={styles.icon}
          />
          <Text style={[styles.searchText, { color: currentTheme.colors.white }]}>{search}</Text>
          <TouchableOpacity
