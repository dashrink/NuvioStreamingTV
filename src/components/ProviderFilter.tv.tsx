...
const ProviderFilter = memo(({
   ...
   */
  const handleItemFocus = useCallback((providerId: string, index: number) => {
   ...
   */
  const handleItemPress = useCallback((providerId: string) => {
    onSelect(providerId);
  }, [onSelect]);
   ...
   */
  const getNextFocusProps = useCallback((index: number) => {
    const props: any = {};
   ...
   */
  const renderItem = useCallback(({ item, index }: { item: ProviderItem; index: number }) => {
   ...
   */
  const getItemLayout = useCallback((data: ArrayLike<ProviderItem> | null | undefined, index: number) => ({
