...
const TabletStreamsLayout: React.FC<TabletStreamsLayoutProps> = ({
  ...
  const backdropSource = React.useMemo(() => {
    ...
    if (!modal && backdropError && episodeImage && episodeImage !== metadata?.poster) {
      if (__DEV__)
        console.log('[TabletStreamsLayout] Episode thumbnail failed, falling back to backdrop');
      if (bannerImage) {
        if (__DEV__)
          console.log('[TabletStreamsLayout] Using show backdrop (episode failed):', bannerImage);
  ...
  useEffect(() => {
    if (backdropSource?.uri && backdropLoaded) {
      ...
      leftPanelOpacity.value = withDelay(
        300,
        withTiming(1, {
          duration: 600,
          easing: Easing.out(Easing.cubic),
      ...
      leftPanelTranslateX.value = withDelay(
        300,
        withTiming(0, {
          duration: 600,
          easing: Easing.out(Easing.cubic),
      ...
      rightPanelOpacity.value = withDelay(
        500,
        withTiming(1, {
          duration: 600,
          easing: Easing.out(Easing.cubic),
      ...
      rightPanelTranslateX.value = withDelay(
        500,
        withTiming(0, {
          duration: 600,
          easing: Easing.out(Easing.cubic),
  ...
  const handleBackdropError = () => {
    if (__DEV__)
      console.log('[TabletStreamsLayout] Backdrop image failed to load:', backdropSource?.uri);
    ...
    if (showNoSourcesError) {
      return (
        <View style={[styles.noStreams, { paddingTop: 50 }]}>
          ...
          <Focusable style={styles.addSourcesButton} onPress={() => navigation.navigate('Addons')}>
    ...
    const renderItem = ({ item }: { item: ListItem }) => {
      ...
      return (
        <StreamCard
          ...
          scraperLogo={
            (stream.addonId && scraperLogos[stream.addonId]) || (stream as any).addon
          ...
          providerName={
            streams &&
            Object.keys(streams).find(pid => (streams as any)[pid]?.streams?.includes?.(stream))
          }
          parentId={id}
  ...
  return (
    <View style={styles.tabletLayout}>
        ...
        <Animated.View style={[styles.tabletLeftPanel, leftPanelAnimatedStyle]}>
          {type === 'movie' && metadata ? (
            ...
            <View style={styles.tabletEpisodeInfo}>
              <Text
                style={[
                  styles.streamsHeroEpisodeNumber,
                  styles.tabletEpisodeText,
                  styles.tabletEpisodeNumber,
                ]}
              >
              ...
              <Text
                style={[
                  styles.streamsHeroTitle,
                  styles.tabletEpisodeText,
                  styles.tabletEpisodeTitle,
                ]}
                numberOfLines={2}
              >
                {currentEpisode.name}
              ...
              {currentEpisode.overview && (
                <Text
                  style={[
                    styles.streamsHeroOverview,
                    styles.tabletEpisodeText,
                    styles.tabletEpisodeOverview,
                  ]}
                  numberOfLines={4}
                >
                  {currentEpisode.overview}
      ...
      <Animated.View
        style={[modal ? styles.tabletModalPanel : styles.tabletRightPanel, rightPanelAnimatedStyle]}
      >
        {Platform.OS === 'android' && AndroidBlurView ? (
          ...
          >
            <AndroidBlurView blurAmount={15} blurRadius={8} style={styles.androidBlurView}>
