...
export const useNextEpisode = (config: NextEpisodeConfig) => {
  ...
  const currentEpisodeDescription = useMemo(() => {
    try {
      ...
      if (episodeId) {
        match = allEpisodes.find(
          ep => ep?.stremioId === episodeId || String(ep?.id) === String(episodeId)
        );
      }
  ...
  const nextEpisode = useMemo(() => {
    try {
      ...
      let nextEp = allEpisodes.find(
        (ep: any) => ep.season_number === season && ep.episode_number === episode + 1
      );

      // If not found, try first episode of next season
      if (!nextEp) {
        nextEp = allEpisodes.find(
          (ep: any) => ep.season_number === season + 1 && ep.episode_number === 1
        );
      }
      return nextEp;
    } catch {
