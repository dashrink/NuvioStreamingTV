...
export const EpisodeStreamsModal: React.FC<EpisodeStreamsModalProps> = ({
  ...
  const [availableStreams, setAvailableStreams] = useState<{
    [providerId: string]: { streams: Stream[]; addonName: string };
  }>({});
  ...
  const fetchStreams = async () => {
    ...
    try {
      ...
      logger.log(
        `[EpisodeStreamsModal] Fetching streams for ${episodeId}, expecting ${expectedProviders.size} providers`
      );
      ...
      await stremioService.getStreams(
        ...
        (streams: any, addonId: any, addonName: any, error: any) => {
          ...
          if (error) {
            setHasErrors(prev => [
              ...prev,
              `${addonName || addonId}: ${error.message || 'Unknown error'}`,
            ]);
        ...
        >
          {isLoading && sortedProviders.length === 0 && (
            <View style={{ padding: 40, alignItems: 'center' }}>
              ...
              <Text style={{ color: 'white', marginTop: 15, opacity: 0.6 }}>
                Finding sources...
              </Text>
            </View>
          )}
                ...
                <View style={{ gap: 8 }}>
                  {providerData.streams.map((stream, index) => {
                    ...
                    const isFirstFocusable =
                      Platform.isTV && sortedProviders[0][0] === providerId && index === 0;
          ...
          {!isLoading && sortedProviders.length === 0 && (
            <View style={{ padding: 40, alignItems: 'center', opacity: 0.5 }}>
              ...
              <Text
                style={{ color: 'white', marginTop: 16, textAlign: 'center', fontWeight: '600' }}
              >
                No sources found
              </Text>
            </View>
          ...
          {hasErrors.length > 0 && (
            ...
            >
              <Text style={{ color: '#EF4444', fontSize: 11 }}>
                Sources might be limited due to provider errors.
              </Text>
            </View>
          )}
        </ScrollView>
