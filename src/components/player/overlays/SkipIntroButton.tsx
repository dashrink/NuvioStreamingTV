...
export const SkipIntroButton: React.FC<SkipIntroButtonProps> = ({
  ...
  useEffect(() => {
    ...
    if (type !== 'series' || !imdbId || !season || !episode) {
      logger.log(
        `[SkipIntroButton] Skipping fetch - type: ${type}, imdbId: ${imdbId}, season: ${season}, episode: ${episode}`
      );
      setIntroData(null);
    ...
    const fetchIntroData = async () => {
      ...
      try {
        ...
        if (data) {
          logger.log(
            `[SkipIntroButton] ✓ Found intro: ${data.start_sec}s - ${data.end_sec}s (confidence: ${data.confidence})`
          );
        } else {
  ...
  const shouldShowButton = useCallback(() => {
    ...
    const inIntroRange =
      currentTime >= introData.start_sec && currentTime < introData.end_sec - 0.5;
  ...
  useEffect(() => {
    ...
    if (shouldShow && !isVisible) {
      logger.log(
        `[SkipIntroButton] Showing button - currentTime: ${currentTime.toFixed(1)}s, intro: ${introData?.start_sec}s - ${introData?.end_sec}s`
      );
      setIsVisible(true);
      ...
      logger.log(
        `[SkipIntroButton] Hiding button - currentTime: ${currentTime.toFixed(1)}s, hasSkipped: ${hasSkipped}`
      );
      if (autoHideTimerRef.current) clearTimeout(autoHideTimerRef.current);
      opacity.value = withTiming(0, { duration: 200 });
      scale.value = withTiming(0.8, { duration: 200 });
  ...
  const handleSkip = useCallback(() => {
    ...
    logger.log(
      `[SkipIntroButton] User pressed Skip Intro - seeking to ${introData.end_sec}s (from ${currentTime.toFixed(1)}s)`
    );
    setHasSkipped(true);
  ...
  return (
    ...
    >
      <TouchableOpacity style={styles.button} onPress={handleSkip} activeOpacity={0.85}>
        <BlurView intensity={60} tint="dark" style={styles.blurContainer}>
          <MaterialIcons name="skip-next" size={20} color="#FFFFFF" style={styles.icon} />
          <Text style={styles.text}>Skip Intro</Text>
          <Animated.View
            style={[styles.accentBar, { backgroundColor: currentTheme.colors.primary }]}
          />
        </BlurView>
      </TouchableOpacity>
    </Animated.View>
