 ...
 * ```
 */
const WatchProgressDisplay = memo(function WatchProgressDisplay({
  watchProgress,
  type,
  getEpisodeDetails,
  animatedStyle,
  ...
  const { isAuthenticated: isTraktAuthenticated, forceSyncTraktProgress } =
    useTraktContext();
  ...
  const progressBoxTranslateY = useSharedValue(
    INITIAL_VALUES.PROGRESS_BOX_TRANSLATE_Y
  );
  ...
  const progressData = useMemo((): ProgressData | null => {
    ...
    if (isWatched) {
      ...
      return {
        ...
        syncStatus:
          isTraktAuthenticated && watchProgress?.traktSynced ? '' : '', // Clean look for watched
        isTraktSynced: Boolean(
          watchProgress?.traktSynced && isTraktAuthenticated
        ),
        isWatched: true,
      };
    }
    ...
    if (isTraktAuthenticated && watchProgress.traktProgress !== undefined) {
      ...
      progressPercent =
        (watchProgress.currentTime / watchProgress.duration) * 100;
    }
    const formattedTime = new Date(
      watchProgress.lastUpdated
  ...
  useEffect(() => {
    if (progressData) {
      ...
      progressBoxTranslateY.value = withTiming(
        INITIAL_VALUES.PROGRESS_BOX_TRANSLATE_Y,
        { duration: PROGRESS_TIMING.EXIT }
      );
    }
  ...
  const isVisible =
    !!progressData && !(isTrailerPlaying && !trailerMuted && trailerReady);
  if (!isVisible) return null;

  const isCompleted =
    progressData.isWatched ||
    progressData.progressPercent >= PROGRESS_THRESHOLDS.LOCAL_WATCHED;
  ...
  return (
    <Animated.View
      style={[
        isTablet
          ? styles.tabletWatchProgressContainer
          : styles.watchProgressContainer,
        animatedStyle,
      ]}
    >
      ...
      <Animated.View
        style={[
          isTablet
            ? styles.tabletProgressGlassBackground
            : styles.progressGlassBackground,
      ...
      >
        ...
        <Animated.View
          style={[styles.watchProgressBarContainer, celebrationAnimatedStyle]}
        >
        ...
        <View style={styles.watchProgressTextContainer}>
          <View style={styles.progressInfoMain}>
            <Text
              style={[
                isTablet
                  ? styles.tabletWatchProgressMainText
                  : styles.watchProgressMainText,
                {
          ...
          {progressData.episodeInfo && (
            <Text
              style={[
                isTablet
                  ? styles.tabletWatchProgressSubText
                  : styles.watchProgressSubText,
                {
                  color: isCompleted
                    ? 'rgba(0,255,136,0.7)'
            ...
            <View style={styles.syncStatusContainer}>
              ...
              <Text
                style={[
                  ...
                  {
                    color: progressData.isTraktSynced
                      ? '#E50914'
                      : 'rgba(255,255,255,0.6)',
                  },
                ]}
