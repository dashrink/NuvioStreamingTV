...
const TrailersSection: React.FC<TrailersSectionProps> = memo(
  ({ tmdbId, type, contentId, contentTitle }) => {
        ...
        try {
          ...
          if (!basicResponse.ok) {
            if (basicResponse.status === 404) {
              ...
              logger.info(
                'TrailersSection',
                `TMDB ID ${tmdbId} not found in TMDB (404) - skipping trailers`
              );
            ...
            logger.error(
              'TrailersSection',
              `TMDB basic endpoint failed: ${basicResponse.status} ${basicResponse.statusText}`
            );
          ...
          if (type === 'movie') {
            ...
            if (!response.ok) {
              ...
              if (response.status === 404) {
                logger.info(
                  'TrailersSection',
                  `No videos found for movie TMDB ID ${tmdbId} (404 response)`
                );
                setTrailers({}); // Empty trailers - section won't render
                return;
              }
              logger.error(
                'TrailersSection',
                `Videos endpoint failed: ${response.status} ${response.statusText}`
              );
            ...
            logger.info(
              'TrailersSection',
              `Received ${allVideos.length} videos for movie TMDB ID ${tmdbId}`
            );
          } else {
            // For TV shows, fetch both main TV videos and season-specific videos
            logger.info(
              'TrailersSection',
              `Fetching TV show videos and season trailers for TMDB ID ${tmdbId}`
            );
            ...
            for (let seasonNum = 1; seasonNum <= numberOfSeasons; seasonNum++) {
              seasonPromises.push(
                fetch(
                  `https://api.themoviedb.org/3/tv/${tmdbId}/season/${seasonNum}/videos?api_key=d131017ccc6e5462a81c9304d21476de&language=en-US`
                )
                  .then(res => res.json())
                  .then(data => ({
                  ...
                  .catch(err => {
                    logger.warn(
                      'TrailersSection',
                      `Failed to fetch season ${seasonNum} videos:`,
                      err
                    );
            ...
            seasonResults.forEach(result => {
              if (result.videos.length > 0) {
                ...
                logger.info(
                  'TrailersSection',
                  `Season ${result.seasonNumber}: ${result.videos.length} videos`
                );
              }
            });

            const totalSeasonVideos = seasonResults.reduce(
              (sum, result) => sum + result.videos.length,
              0
            );
            logger.info(
              'TrailersSection',
              `Total videos collected: ${allVideos.length} (main: ${allVideos.filter(v => v.seasonNumber === null).length}, seasons: ${totalSeasonVideos})`
            );
          }

          const categorized = categorizeTrailers(allVideos);
          const totalVideos = Object.values(categorized).reduce(
          ...
          if (totalVideos === 0) {
            logger.info(
              'TrailersSection',
              `No videos found for TMDB ID ${tmdbId} - this is normal`
            );
            ...
            logger.info(
              'TrailersSection',
              `Categorized ${totalVideos} videos into ${Object.keys(categorized).length} categories`
            );
            ...
            const preferredCategory = availableCategories.includes('Trailer')
              ? 'Trailer'
              : availableCategories.includes('Teaser')
                ? 'Teaser'
    ...
    return (
      <Animated.View
        style={[styles.container, sectionAnimatedStyle, { paddingHorizontal: horizontalPadding }]}
      >
        ...
        {selectedCategory && trailers[selectedCategory] && (
          <View style={styles.selectedCategoryContent}>
            ...
            <ScrollView
              ...
              contentContainerStyle={[styles.trailersScrollContent, { gap: trailerCardSpacing }]}
            ...
            >
              {trailers[selectedCategory].map((trailer, index) => (
                <View
                  key={trailer.id}
                  style={[styles.trailerCardContainer, { width: trailerCardWidth }]}
                >
                  <TouchableOpacity
                    style={[
                      styles.trailerCard,
              ...
              {trailers[selectedCategory].length >
                (isTV ? 5 : isLargeTablet ? 4 : isTablet ? 4 : 3) && (
                <View
                  style={[
