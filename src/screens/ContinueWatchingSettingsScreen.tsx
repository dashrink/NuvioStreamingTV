...
const ContinueWatchingSettingsScreen: React.FC = () => {
  ...
  const SettingItem = ({
    ...
    >
      <View style={styles.settingContent}>
        <Text style={[styles.settingTitle, { color: colors.highEmphasis }]}>{title}</Text>
  ...
  const TTLPickerItem = ({ option }: { option: { label: string; value: number } }) => {
    ...
    return (
      ...
      >
        <Text
          style={[styles.ttlOptionText, { color: isSelected ? colors.white : colors.highEmphasis }]}
        >
          {option.label}
        </Text>
        ...
        {settings.useCachedStreams && (
          <View style={styles.section}>
            ...
            <View style={styles.settingsCard}>
              ...
              >
                <Text
                  style={[styles.settingTitle, { color: colors.highEmphasis, marginBottom: 8 }]}
                >
                ...
                <Text
                  style={[
                    ...
                    { color: colors.mediumEmphasis, marginBottom: 16 },
                  ]}
                >
          ...
          <View style={styles.section}>
            <View style={[styles.warningCard, { borderColor: colors.warning }]}>
              <View style={styles.warningHeader}>
                ...
                <Text style={[styles.warningTitle, { color: colors.warning }]}>Important Note</Text>
              </View>
              <Text style={[styles.warningText, { color: colors.mediumEmphasis }]}>
                Not all stream links may remain active for the full cache duration. Longer cache
                times may result in expired links. If a cached link fails, the app will fall back to
        ...
        <View style={styles.section}>
          <View style={styles.infoCard}>
            <View style={styles.infoHeader}>
              ...
              <Text style={[styles.infoTitle, { color: colors.highEmphasis }]}>How it works</Text>
            </View>
            <Text style={[styles.infoText, { color: colors.mediumEmphasis }]}>
              {settings.useCachedStreams ? (
                <>
                  • Streams are cached for your selected duration after playing{'\n'}• Cached
                  streams are validated before use{'\n'}• If cache is invalid or expired, falls back
                  to content screen{'\n'}• "Use Cached Streams" controls direct player vs screen
                  navigation{'\n'}• "Open Metadata Screen" appears only when cached streams are
                  disabled
                </>
              ) : (
                <>
                  • When cached streams are disabled, clicking Continue Watching items opens content
