...
const getPlanName = (plan: number): string => {
  switch (plan) {
    case 0:
      return 'Free';
    case 1:
      return 'Essential ($3/mo)';
    case 2:
      return 'Pro ($10/mo)';
    case 3:
      return 'Standard ($5/mo)';
    default:
      return 'Unknown';
  }
};
...
const DebridIntegrationScreen = () => {
  ...
  const loadTorrentioConfig = useCallback(async () => {
    try {
      ...
      const addons = await stremioService.getInstalledAddonsAsync();
      const torrentioAddon = addons.find(
        addon =>
  ...
  const handleDisconnect = async () => {
    ...
    setAlertMessage(
      'Are you sure you want to disconnect Torbox? This will remove the addon and clear your saved API key.'
    );
    setAlertActions([
      {
        label: 'Cancel',
  ...
  const openSubscription = () => {
    Linking.openURL(
      'https://torbox.app/subscription?referral=493192f2-6403-440f-b414-768f72222ec7'
    );
  };
  ...
  const generateTorrentioManifestUrl = useCallback((): string => {
    ...
    if (
      torrentioConfig.providers.length > 0 &&
      torrentioConfig.providers.length < TORRENTIO_PROVIDERS.length
    ) {
    ...
    if (torrentioConfig.debridService !== 'none' && torrentioConfig.debridApiKey) {
      const debridInfo = TORRENTIO_DEBRID_SERVICES.find(
        d => d.id === torrentioConfig.debridService
      );
      if (debridInfo) {
  ...
  const handleInstallTorrentio = async () => {
    ...
    try {
      ...
      const addons = await stremioService.getInstalledAddonsAsync();
      const existingTorrentio = addons.find(
        addon =>
          addon.id?.includes('torrentio') ||
          addon.url?.includes('torrentio.strem.fun') ||
          (addon as any).transport?.includes('torrentio.strem.fun')
      ...
      {config?.isConnected ? (
        <>
          ...
          {userData && (
            <View style={styles.userDataCard}>
              <View style={styles.userDataHeader}>
                ...
                {userDataLoading && <ActivityIndicator size="small" color={colors.primary} />}
              </View>
          ...
          <View style={styles.section}>
            ...
            <Text style={styles.sectionText}>
              Your TorBox addon is active and providing premium streams.
              {config.isEnabled ? '' : ' (Currently disabled)'}
            </Text>
          </View>
          ...
          <View style={styles.section}>
            ...
            <Text style={styles.sectionText}>
              Customize your streaming experience. Sort by quality, filter file sizes, and manage
        ...
        <>
          <Text style={styles.description}>
            Unlock 4K high-quality streams and lightning-fast speeds by integrating Torbox. Enter
            your API Key below to instantly upgrade your streaming experience.
          </Text>
          ...
          <TouchableOpacity
            onPress={() => {
              triggerLight();
              Linking.openURL(
                'https://guides.viren070.me/stremio/technical-details#debrid-services'
              );
            }}
  ...
  const renderTorrentioTab = () => (
    <>
      <Text style={styles.description}>
        Configure Torrentio to get torrent streams for movies and TV shows. A debrid service is
      ...
      {!torrentioConfig.debridApiKey && (
        <View style={styles.promoCard}>
          ...
          <Text style={styles.promoText}>
            Get TorBox for lightning-fast 4K streaming with zero buffering. Premium cached torrents
      ...
      >
        ...
        <Feather
          name={expandedSections.sorting ? 'chevron-up' : 'chevron-down'}
          size={20}
          color={colors.mediumEmphasis}
        />
      </TouchableOpacity>
          ...
          <View style={styles.pickerContainer}>
            {TORRENTIO_SORT_OPTIONS.map(option => (
              <TouchableOpacity
                ...
                style={[
                  styles.pickerItem,
                  torrentioConfig.sort === option.id && styles.pickerItemSelected,
                ]}
                onPress={() => {
                  triggerLight();
              ...
              >
                <Text
                  style={[
                    styles.pickerItemText,
                    torrentioConfig.sort === option.id && styles.pickerItemTextSelected,
                  ]}
                >
                  {option.name}
                </Text>
      ...
      >
        <View>
          ...
          <Text style={styles.accordionSubtext}>
            {torrentioConfig.qualityFilter.length > 0
              ? `${torrentioConfig.qualityFilter.length} excluded`
              : 'None excluded'}
          </Text>
        </View>
        <Feather
          name={expandedSections.qualityFilter ? 'chevron-up' : 'chevron-down'}
          size={20}
          color={colors.mediumEmphasis}
        />
      </TouchableOpacity>
          ...
          <View style={styles.chipContainer}>
            {TORRENTIO_QUALITY_FILTERS.map(quality => (
              <TouchableOpacity
                ...
                style={[
                  styles.chip,
                  torrentioConfig.qualityFilter.includes(quality.id) && styles.chipSelected,
                ]}
              ...
              >
                <Text
                  style={[
                    styles.chipText,
                    torrentioConfig.qualityFilter.includes(quality.id) && styles.chipTextSelected,
                  ]}
                >
                  {quality.name}
                </Text>
      ...
      >
        <View>
          ...
          <Text style={styles.accordionSubtext}>
            {torrentioConfig.priorityLanguages.length > 0
              ? `${torrentioConfig.priorityLanguages.length} selected`
              : 'No preference'}
          </Text>
        </View>
        <Feather
          name={expandedSections.languages ? 'chevron-up' : 'chevron-down'}
          size={20}
          color={colors.mediumEmphasis}
        />
      </TouchableOpacity>
          ...
          <View style={styles.chipContainer}>
            {TORRENTIO_LANGUAGES.map(lang => (
              <TouchableOpacity
                ...
                style={[
                  styles.chip,
                  torrentioConfig.priorityLanguages.includes(lang.id) && styles.chipSelected,
                ]}
              ...
              >
                <Text
                  style={[
                    styles.chipText,
                    torrentioConfig.priorityLanguages.includes(lang.id) && styles.chipTextSelected,
                  ]}
                >
                  {lang.name}
                </Text>
      ...
      >
        <View>
          ...
          <Text style={styles.accordionSubtext}>
            {TORRENTIO_MAX_RESULTS.find(o => o.id === torrentioConfig.maxResults)?.name ||
              'All results'}
          </Text>
        </View>
        <Feather
          name={expandedSections.maxResults ? 'chevron-up' : 'chevron-down'}
          size={20}
          color={colors.mediumEmphasis}
        />
      </TouchableOpacity>
          ...
          <View style={styles.pickerContainer}>
            {TORRENTIO_MAX_RESULTS.map(option => (
              <TouchableOpacity
                ...
                style={[
                  styles.pickerItem,
                  torrentioConfig.maxResults === option.id && styles.pickerItemSelected,
                ]}
                onPress={() => {
                  triggerLight();
              ...
              >
                <Text
                  style={[
                    styles.pickerItemText,
                    torrentioConfig.maxResults === option.id && styles.pickerItemTextSelected,
                  ]}
                >
                  {option.name}
                </Text>
      ...
      >
        ...
        <Feather
          name={expandedSections.options ? 'chevron-up' : 'chevron-down'}
          size={20}
          color={colors.mediumEmphasis}
        />
      </TouchableOpacity>
        ...
        <View style={styles.accordionContent}>
          <View style={styles.switchRow}>
            ...
            <Switch
              ...
              onValueChange={val => {
                triggerMedium();
                setTorrentioConfig(prev => ({ ...prev, noDownloadLinks: val }));
              }}
          ...
          <View style={styles.switchRow}>
            ...
            <Switch
              ...
              onValueChange={val => {
                triggerMedium();
                setTorrentioConfig(prev => ({ ...prev, noCatalog: val }));
              }}
              trackColor={{ false: colors.elevation3, true: colors.primary }}
              thumbColor={colors.white}
            />
          ...
          <>
            ...
            <TouchableOpacity
              style={[
                styles.actionButton,
                styles.dangerButton,
                torrentioLoading && styles.disabledButton,
              ]}
              onPress={() => {
                triggerHeavy();
                handleRemoveTorrentio();
              }}
              disabled={torrentioLoading}
            >
