...
import { useNavigation, useRoute, RouteProp , NavigationProp , StackActions } from '@react-navigation/native';
...
const CastMoviesScreen: React.FC = () => {
      ...
      if (credits && credits.cast) {
        ...
        const allCredits = credits.cast
          .filter((item: any) => {
            ...
            const isTalkShow = talkShowKeywords.some(
      ...
      <Animated.View entering={FadeIn.delay(100)}>
        <TouchableOpacity
          style={{
            ...
            paddingVertical: 10,
            borderRadius: 25,
            backgroundColor: isSelected ? currentTheme.colors.primary : 'rgba(255, 255, 255, 0.08)',
            marginRight: 12,
  ...
  const renderSortButton = (
    sort: 'popularity' | 'latest' | 'upcoming',
    label: string,
    icon: string
  ) => {
  ...
  const renderMovieItem = useCallback(
    ({ item, index }: { item: CastMovie; index: number }) => (
      <Animated.View
