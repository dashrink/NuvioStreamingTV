...
const HDR_ICON =
  'https://uxwing.com/wp-content/themes/uxwing/download/video-photography-multimedia/hdr-icon.png';
const DOLBY_ICON =
  'https://upload.wikimedia.org/wikipedia/en/thumb/3/3f/Dolby_Vision_%28logo%29.svg/512px-Dolby_Vision_%28logo%29.svg.png?20220908042900';
...
const QualityTag = React.memo(
  ({ text, color, theme }: { text: string; color: string; theme: any }) => {
...
export const StreamsScreen = () => {
  ...
  useEffect(() => {
    ...
    const currentStreamsData =
      metadata?.videos && metadata.videos.length > 1 && selectedEpisode
        ...
        : groupedStreams;
    if (__DEV__)
      console.log('[StreamsScreen] streams state changed', {
        providerKeys: Object.keys(currentStreamsData || {}),
        type,
      });
    ...
    if (providersWithStreams.length > 0) {
      ...
      if (hasNewProviders) {
        setAvailableProviders(prevProviders => {
          ...
          if (__DEV__)
            console.log('[StreamsScreen] availableProviders ->', Array.from(newProviders));
          return newProviders;
        });
      }
    }
  ...
  useEffect(() => {
    ...
    const checkProviders = async () => {
      if (__DEV__)
        console.log('[StreamsScreen] checkProviders() start', { id, type, episodeId, fromPlayer });
      logger.log(
        `[StreamsScreen] checkProviders() start id=${id} type=${type} episodeId=${episodeId || 'none'} fromPlayer=${!!fromPlayer}`
      );
      ...
      try {
        ...
        if (__DEV__)
          console.log(
            '[StreamsScreen] hasStremioProviders:',
            hasStremioProviders,
            'for type:',
            type
          );
        ...
        if (__DEV__)
          console.log(
            '[StreamsScreen] hasLocalScrapers:',
            hasLocalScrapers,
            'enableLocalScrapers:',
        ...
        logger.log(
          `[StreamsScreen] provider check: hasProviders=${hasProviders} (stremio:${hasStremioProviders}, local:${hasLocalScrapers})`
        );
  ...
  const filterStreamsByLanguage = useCallback(
    (streams: Stream[]) => {
      ...
      console.log(
        '🔍 [filterStreamsByLanguage] Filtering with excluded languages:',
      ...
      if (streams.length > 0) {
        console.log(
          '🔍 [filterStreamsByLanguage] Sample stream details:',
      ...
      const filtered = streams.filter(stream => {
        ...
        const hasExcludedLanguage = settings.excludedLanguages.some(excludedLanguage => {
          ...
          if (matches) {
            console.log(
              `🔍 [filterStreamsByLanguage] ✕ Excluding stream with ${excludedLanguage}:`,
              streamName.substring(0, 100)
            );
          }
          return matches;
        });
      ...
      console.log(
        `🔍 [filterStreamsByLanguage] Filtered ${streams.length} → ${filtered.length} streams`
      );
      return filtered;
    },
    [settings.excludedLanguages]
  ...
  const getBestStream = useCallback(
    (streamsData: typeof groupedStreams): Stream | null => {
      ...
      logger.log(
        `🎯 Best stream selected: ${allStreams[0].stream.name || allStreams[0].stream.title} (Quality: ${allStreams[0].quality}p, Provider Priority: ${allStreams[0].providerPriority})`
      );
        ...
        if (stream.url) {
          ...
          try {
            if (Platform.OS === 'ios' && settings.preferredPlayer === 'internal') {
              ...
              const contentType =
                (stream.headers &&
                  ((stream.headers as any)['Content-Type'] ||
                    (stream.headers as any)['content-type'])) ||
                '';
              const isMkvByHeader =
                typeof contentType === 'string' && contentType.includes('matroska');
              const isMkvByPath =
                lowerUri.includes('.mkv') ||
                /[?&]ext=mkv\b/.test(lowerUri) ||
                /format=mkv\b/.test(lowerUri) ||
                /container=mkv\b/.test(lowerUri);
            ...
            if (!isMkvByPath && isHttp) {
              try {
                ...
                if (mkvDetected) {
                  ...
                  logger.log(
                    '[StreamsScreen] HEAD detected MKV via Content-Type - will play in KSPlayer on iOS'
                  );
            ...
            try {
              ...
              const tryNextUrl = (index: number) => {
                if (index >= externalPlayerUrls.length) {
                  if (__DEV__)
                    console.log(
                      `All ${settings.preferredPlayer} formats failed, falling back to direct URL`
                    );
                  ...
                  Linking.openURL(stream.url)
                    .then(() => {
                      if (__DEV__) console.log('Opened with direct URL');
                    })
                    .catch(() => {
                      if (__DEV__)
                ...
                Linking.openURL(url)
                  .then(() => {
                    if (__DEV__)
                      console.log(
                        `Successfully opened stream with ${settings.preferredPlayer} format ${index + 1}`
                      );
                  })
                  .catch(err => {
          ...
          // For Android with external player preference
            ...
            try {
              ...
              if (isMagnet) {
                ...
                Linking.openURL(stream.url)
                  .then(() => {
                    if (__DEV__) console.log('Successfully opened magnet link');
                  })
                  .catch(err => {
  ...
  const episodeImage = useMemo(() => {
    ...
    metadata,
    episodeThumbnail,
    tmdbEpisodeOverride?.still_path,
    settings.enrichMetadataWithTMDB,
  ]);
  ...
  const effectiveEpisodeRuntime = useMemo(() => {
    ...
    const r = (tmdbEpisodeOverride?.runtime ?? (currentEpisode as any).runtime) as
      | number
      | undefined;
    return r;
                              ...
                              <View>
                                <StreamCard
                                  ...
                                  scraperLogo={
                                    ...
                                    (item as any).addon
                                      ? scraperLogoCache.get(
                                          (item.addonId || (item as any).addon) as string
                                        ) || null
                                      : null
                                  }
